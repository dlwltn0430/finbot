...
async def search_savings_by_target_amount(
        ...
        "$arrayElemAt": [{
            "$map": {
                "input": {
                    "$filter": {
                        "input": "$value",
                        "as": "baseInterestRateTier",
                        "cond": {
                            "$and": [
                                {
                                    "$gte": [
                                        "$$baseInterestRateTier.min_term",
                                        total_term_months,
                                    ]
                                },
                                {
                                    "$lt": [
                                        "$$baseInterestRateTier.max_term",
                                        total_term_months,
                                    ]
                                },
                            ]
                ...
                "as": "filteredBaseInterestRate",
                "in": "$$filteredBaseInterestRate.interest_rate"
            },
        }, 0]
    }
    ...
    # 중간 금리 (기본 금리 + user_choice 우대금리 합)
    intermediate_interest_rate = {
        "intermediateInterestRate": {
            "$sum": {
                "$map": {
                    "input": {
                        "$filter": {
                            "input": "$preferential_rates",
                            "as": "preferential_rate_item",
                            "cond": {
                                "$eq": ["$$preferential_rate_item."]
                            }
                        }
                    }
                }
    ...
    # 최대 금리 (기본 금리 + 전체 우대금리 합)
    max_interest_rate = {}

    preferential_rate_pipelines = []
...
async def search_savings(
    ...
    if financial_calculations:
        # MongoDB 5.0 이상에서는 $function을 사용하여 복잡한 JS 함수 실행 가능
        # 간결성을 위해 maturity_details 내 반복되는 JS 함수 본문은 "..."로 축약
        # 실제 코드에서는 전체 함수 본문을 복사하여 사용해야 합니다.
